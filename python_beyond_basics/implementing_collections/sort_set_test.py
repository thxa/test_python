import unittest
from sorted_set import SortedSet
from collections.abc import (Sequence, Sized, Container, Iterable, Set)


# Construction
class TestConstruction(unittest.TestCase):

	def test_empty(self):
		s = SortedSet([])

	def test_from_sequence(self):
		s = SortedSet([7, 2, 4, 5 , 12, 3])

	def test_with_duplicates(self):
		s = SortedSet([1, 1 , 1])

	def test_from_iterable(self):
		def gen6842():
			yield 6
			yield 8
			yield 4
			yield 2
		g = gen6842()
		s = SortedSet(g)

	def test_default_empty(self):
		s = SortedSet()


# ContainerProtocol
class TestContainerProtocol(unittest.TestCase):

	def setUp(self):
		self.s = SortedSet([5, 20, 3, 6, -1])

	def test_positve_contained(self):
		self.assertTrue(6 in self.s)

	def test_negative_contained(self):
		self.assertFalse(2 in self.s)

	def test_positve_not_contained(self):
		self.assertTrue(4 not in self.s)

	def test_negative_not_contained(self):
		self.assertFalse(6 not in self.s)

	def test_protocol(self):
		self.assertTrue(issubclass(SortedSet, Container))


# SizedProtocol
class TestSizedProtocol(unittest.TestCase):

	def test_empty(self):
		s = SortedSet()
		self.assertEqual(len(s), 0)

	def test_one(self):
		s = SortedSet([33])
		self.assertEqual(len(s), 1)

	def test_ten(self):
		s = SortedSet(range(10))
		self.assertEqual(len(s), 10)

	def test_with_duplicates(self):
		s = SortedSet([1, 1, 1])
		self.assertEqual(len(s), 1)

	def test_protocol(self):
		self.assertTrue(issubclass(SortedSet, Sized))


# IterableProtocol
class TestIterableProtocol(unittest.TestCase):

	def setUp(self):
		self.s = SortedSet([7, 4, 5, 2])

	def test_iter(self):
		i = iter(self.s)
		self.assertEqual(next(i), 2)
		self.assertEqual(next(i), 4)
		self.assertEqual(next(i), 5)
		self.assertEqual(next(i), 7)
		self.assertRaises(StopIteration, lambda: next(i))

	def test_for_loop(self):
		index = 0
		expected = [2, 4, 5, 7]
		for item in self.s:
			self.assertEqual(item, expected[index])
			index += 1

	def test_protocol(self):
		self.assertTrue(issubclass(SortedSet, Iterable))


# SequenceProtocol
class TestSequenceProtocol(unittest.TestCase):

	def setUp(self):
		self.s = SortedSet([5, 3, 1, 15])

	def test_index_zero(self):
		self.assertEqual(self.s[0], 1)

	def test_index_four(self):
		self.assertEqual(self.s[3], 15)

	def test_index_one_beyond_the_end(self):
		with self.assertRaises(IndexError):
			self.s[4]

	def test_index_minus_one(self):
		self.assertEqual(self.s[-1], 15)

	def test_index_minus_five(self):
		self.assertEqual(self.s[-4], 1)

	def test_index_one_before_the_beginning(self):
		with self.assertRaises(IndexError):
			self.s[-5]
	
	# slice
	def test_slice_form_start(self):
		self.assertEqual(self.s[:2], SortedSet([1, 3]))

	def test_slice_form_end(self):
		self.assertEqual(self.s[2:], SortedSet([5, 15]))

	def test_slice_empty(self):
		self.assertEqual(self.s[10:], SortedSet())

	def test_slice_arbitary(self):
		self.assertEqual(self.s[2:3], SortedSet([5]))

	def test_slice_full(self):
		self.assertEqual(self.s[:], self.s)

	def test_reversed(self):
		s = SortedSet([3, 2, 4, 1])
		r = reversed(s)
		self.assertEqual(next(r), 4)
		self.assertEqual(next(r), 3)
		self.assertEqual(next(r), 2)
		self.assertEqual(next(r), 1)
		with self.assertRaises(StopIteration):
			next(r)

	def test_index_positive(self):
		s = SortedSet([2, 5, 3, 1])
		self.assertEqual(s.index(3), 2)

	def test_index_negative(self):
		s = SortedSet([2, 5, 3, 1])
		with self.assertRaises(ValueError):
			s.index(20)

	def test_count_zero(self):
		s = SortedSet([1, 5, 7, 9])
		self.assertEqual(s.count(10), 0)

	def test_count_one(self):
		s = SortedSet([1, 5, 7, 9])
		self.assertEqual(s.count(5), 1)

	def test_protocol(self):
		self.assertTrue(issubclass(SortedSet, Sequence))

	def test_concatenate_disjoint(self):
		s = SortedSet([2, 1, 5])
		t = SortedSet([4, 3, 6])
		self.assertEqual(s + t, SortedSet([2, 1, 5, 4, 3, 6]))

	def test_concatenate_equal(self):
		s = SortedSet([2, 4, 6])
		self.assertEqual(s + s , s)

	def test_concatenate_intersecting(self):
		s = SortedSet([2, 1, 5])
		t = SortedSet([4, 3, 5])
		self.assertEqual(s + t, SortedSet([1, 2, 3, 4, 5]))

	def test_repetition_zero(self):
		s = SortedSet([4, 5, 6])
		self.assertEqual(s * 0, SortedSet())

	def test_repetition_nonzero(self):
		s = SortedSet([4, 5, 6])
		self.assertEqual(s * 100, s)

	def test_repetition_zero_left(self):
		s = SortedSet([4, 5, 6])
		self.assertEqual(0 * s, SortedSet())

	def test_repetition_nonzero_left(self):
		s = SortedSet([4, 5, 6])
		self.assertEqual(100 * s, s)
		

# ReprProtocol
class TestReprProtocol(unittest.TestCase):

	def test_repr_empty(self):
		s = SortedSet([])
		self.assertEqual(repr(s), "SortedSet()")

	def test_repr_some(self):
		s = SortedSet([42, 44, 3])
		self.assertEqual(repr(s), "SortedSet([3, 42, 44])")


# EqualityProtocol
class TestEqualityProtocol(unittest.TestCase):

	def test_positive_equal(self):
		self.assertTrue(SortedSet([4, 5, 6]) == SortedSet([6, 5, 4]))

	def test_negative_equal(self):
		self.assertFalse(SortedSet([4, 5, 6]) == SortedSet([3, 2, 7]))

	def test_type_mismatch(self):
		self.assertFalse(SortedSet([4, 5, 6]) == [4, 5, 6])

	def test_identical(self):
		s = SortedSet([4, 1, 8])
		self.assertTrue(s == s)


# InequalityProtocol
class TestInequalityProtocol(unittest.TestCase):

	def test_positive_inequal(self):
		self.assertTrue(SortedSet([4, 5, 6]) != SortedSet([2, 5, 4]))

	def test_negative_inequal(self):
		self.assertFalse(SortedSet([4, 5, 6]) != SortedSet([4, 5, 6]))

	def test_type_mismatch(self):
		self.assertTrue(SortedSet([1, 2, 3]) != [1, 2, 3])

	def test_identical(self):
		s = SortedSet([4, 1, 8])
		self.assertFalse(s != s)


# TestRelationalSetProtocol [<, >, <=, >=]
class TestRelationalSetProtocol(unittest.TestCase):

	def test_lt_positive(self):
		s = SortedSet({1, 2})
		t = SortedSet({1, 2, 3})
		self.assertTrue(s < t)

	def test_lt_negative(self):
		s = SortedSet({1, 2, 3})
		t = SortedSet({1, 2, 3})
		self.assertFalse(s < t)

	def test_lt_eq_positive(self):
		s = SortedSet({1, 2, 3})
		t = SortedSet({1, 2, 3})
		self.assertTrue(s <= t)

	def test_le_negative(self):
		s = SortedSet({1, 2, 3})
		t = SortedSet({1, 2})
		self.assertFalse(s <= t)

	def test_gt_positive(self):
		s = SortedSet({1, 2, 3})
		t = SortedSet({1, 2})
		self.assertTrue(s > t)

	def test_gt_negative(self):
		s = SortedSet({1, 2})
		t = SortedSet({1, 2, 3})
		self.assertFalse(s > t)

	def test_ge_positive(self):
		s = SortedSet({1, 2, 3})
		t = SortedSet({1, 2, 3})
		self.assertTrue(s >= t)

	def test_ge_negative(self):
		s = SortedSet({1, 2})
		t = SortedSet({1, 2, 3})
		self.assertFalse(s >= t)


class TestRelationalSetMethods(unittest.TestCase):

	def test_issubset_proper_positive(self):
		s = SortedSet({1, 2, 3})
		t = [1, 2, 3]
		self.assertTrue(s.issubset(t))

	def test_issubset_positive(self):
		s = SortedSet({1, 2})
		t = [1, 2, 3]
		self.assertTrue(s.issubset(t))

	def test_issubset_negative(self):
		s = SortedSet({1, 2, 3})
		t = [1, 2]
		self.assertFalse(s.issubset(t))

	def test_issuperset_proper_positive(self):
		s = SortedSet({1, 2, 3})
		t = [1, 2, 3]
		self.assertTrue(s.issuperset(t))

	def test_issuperset_positive(self):
		s = SortedSet({1, 2, 3})
		t = [1, 2]
		self.assertTrue(s.issuperset(t))

	def test_issuperset_negative(self):
		s = SortedSet({1, 2})
		t = [1, 2, 3]
		self.assertFalse(s.issuperset(t))


class TestOperationsMethods(unittest.TestCase):

	def test_intersection(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s.intersection(t), SortedSet({2, 3}))

	def test_union(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s.union(t), SortedSet({1, 2, 3, 4}))

	def test_symmetric_difference(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s.symmetric_difference(t), SortedSet({1, 4}))
	
	def test_difference(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s.difference(t), SortedSet({1}))

	def test_isdisjoint_positve(self):
		s = SortedSet({1, 2, 3})
		t = [4, 5, 6]
		self.assertTrue(s.isdisjoint(t))

	def test_isdisjoint_negative(self):
		s = SortedSet({1, 2, 3})
		t = [3, 4, 5]
		self.assertFalse(s.isdisjoint(t))


class TestOperationsSet(unittest.TestCase):

	def test_intersection(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]	
		self.assertEqual(s & t, SortedSet({2, 3}))

	def test_union(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s | t, SortedSet({1, 2, 3, 4}))

	def test_symmetric_difference(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s ^ t, SortedSet({1, 4}))
	
	def test_difference(self):
		s = SortedSet({1, 2, 3})
		t = [2, 3, 4]
		self.assertEqual(s - t, SortedSet({1}))


class TestSetProtocol(unittest.TestCase):

	def test_protocol(self):
		self.assertTrue(issubclass(SortedSet, Set))


class TestMutableset(unittest.TestCase):
	
	def test_copy_method(self):
		s = SortedSet({1, 2, 3, 4})
		t = s.copy()
		self.assertTrue(id(s) != id(t))
		self.assertEqual(t, s)


from random import randrange
from timeit import timeit

if __name__ == '__main__':
	unittest.main()

	s = SortedSet(randrange(1000) for _ in range(2000))
	print(len(s))
	print(timeit(setup="from __main__ import s",
		   stmt='[s.count(i) for i in range(1000)]', number=100))